...
public class LatestInformationDecoration extends RecyclerView.ItemDecoration {
  ...
  public LatestInformationDecoration(Context context) {
    Resources resources = context.getResources();
    lineSize = (int) resources.getDimension(R.dimen.h_settings_list_item_divider);
    drawable = new ColorDrawable(Color.BLACK);
    paddingLeft = (int) resources.getDimension(R.dimen.p_l_r_layout_latest_information_rv_list_item);
    paddingRight = (int) resources.getDimension(R.dimen.p_l_r_layout_latest_information_rv_list_item);
  }
  ...
  public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
    ...
    if ( layoutManager instanceof LinearLayoutManager &&
      ...
      if ( orientation == LinearLayoutManager.VERTICAL ) {//垂直
        if ( current == lastPosition || current == 0 || current == 1) {
          outRect.set(0, 0, 0, 0);
        } else {
